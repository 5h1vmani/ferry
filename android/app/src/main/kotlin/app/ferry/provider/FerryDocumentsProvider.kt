...
private fun keyOf(documentId: String): String = documentId.substringBefore(PATH_SEPARATOR)

// The root-relative path, which is everything after the first slash. Empty
// for a device's own document.
private fun pathOf(documentId: String): String = documentId.substringAfter(PATH_SEPARATOR, "")

private fun documentIdOf(keyHex: String, path: String): String =
...
private fun parentPathOf(path: String): String = path.substringBeforeLast(PATH_SEPARATOR, "")
...
private fun flagsOf(depth: Int, directory: Boolean): Int = when (depth) {
    0 -> 0
...
private fun refused(documentId: String, error: FerryException): FileNotFoundException {
