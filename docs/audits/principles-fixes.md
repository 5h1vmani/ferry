...
Commit: 5e30ac6, the head of main. The range read is `a044df4..5e30ac6`, 57
commits.
Scope: eight areas of the principles fix batch. They are the shared on-disk
write module (8e4b5df), the shared wire helpers (5fdd17b), the new
`landing_folder` engine call (6dfd5eb), the Mac threading split (4d64318 and
d5c3b0a), the phone threading and lifecycle split (2733f7a, dc23f7d, bec8841,
b080a50, 34b9d64, 9b34cbc), the adb timeout change (47ec347), the test
changes (a90036e, 2026d63), and the CI workflow (55624f7, 4694337). The
...
- The five on-disk stores are behaviour-identical after 8e4b5df. I compared
...
- `ShareIntake.launchOnIo` reports a failure the same way the old bare
  thread did. Commit 2733f7a changed only `Thread {` to
...
- The cargo cache step is unchanged. The paths and the key at `ci.yml:25-30`
  are the same text as at a044df4, so it still keys on `**/Cargo.lock` and
  `rust-toolchain.toml`.
...
2, 4, 9, and 10 are commits 2d22e56 to 97aa721 on `fix-mac`. Phone rows 5,
6, 8, and 14 are 0ffbdf7 to 33ce40a on `fix-phone`. Engine, test, and CI
rows 3, 7, 11, 12, 13, 15, and 16 are cac6a62 to 8bf4949 on `fix-rust`.
